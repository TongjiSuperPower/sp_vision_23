...
class NahsorSolver:
    ...
    def solve(self, nahsor: NahsorMarker, yaw_degree: float, pitch_degree: float):
        R_gimbal2imu = tools.R_gimbal2imu(yaw_degree, pitch_degree)

        def lazy_solve(nahsor: NahsorMarker):
            points_2d = nahsor.target_points
            points_2d.append(nahsor.r_center)
            # 3D坐标由能量机关尺寸图计算出
            points_3d = np.float32([[-186, 36, 0],
                                    [-160, 353, 0],
                                    [160, 353, 0],
                                    [186, 36, 0],
                                    [0, -501, 0]])
